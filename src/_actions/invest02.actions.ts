...
export const investment02Deposit =
  ...
  async (dispatch: Dispatch<AppActions>, getState: () => AppState) => {
    ...
    if (token !== 'BNB')
      allowance = await tokenContract[token].allowance(
        address,
        info[chainId].STT
      )
    ...
    if (token !== 'BNB' && bytesFormater(allowance) === 0) {
      dispatch({
        type: INVEST02_METHOD_REQUEST,
        payload: { method: 'approve' },
      })
...
export const withdrawInterest =
  (address?: string) => async (dispatch: Dispatch<AppActions>) => {
